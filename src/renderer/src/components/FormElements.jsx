...
const Input = ({
  ...
  return (
    <div className="w-full">
      <label className="mb-2.5 block font-medium text-black dark:text-bodydark1">{label}</label>
      ...
      <span className="text-red-400 font-medium">{errors?.[name]?.message}</span>
    </div>
  )
}
...
const DatePicker = ({
  ...
  return (
    <div className="w-full">
      <label
        className={`${label.length > 0 && 'mb-3 block font-medium text-black dark:text-white'}`}
      >
        {label}
      </label>
      <div className="relative">
